...
/// أقصى عرض للمحتوى.
///
/// ⚠️ الرقم ده كان ١٨٠٠ — يعني على شاشة ١٩٢٠ مابيعملش حاجة تقريباً،
/// والكارت بيفضل مفرود بعرض الشاشة كلها. الحل الحقيقي إن القوايم تتقسم
/// **أعمدة** (شوف `ResponsiveCards`)، والحد ده بيبقى شبكة أمان للشاشات
/// العملاقة (٢٥٦٠ وفوق) اللي حتى ٣ أعمدة فيها بتبقى عريضة.
const double kContentMaxWidth = 2100;
...
extension BpContext on BuildContext {
  ...
  /// 🚫 مطفّية — اتجرّبت تلات مرات وفشلت.
  ///
  /// الشكل: القايمة والمحتوى جنب بعض في `Row`. النتيجة **شاشة سودا**
  /// على الشاشة العريضة (تاب Samsung A9+ بالعرض = 1280 نقطة).
  ///
  /// 🔬 التحقيق الأخير (٢٠٢٦-٠٨) — شغّلت البرنامج فعلاً على ويندوز
  /// وصوّرت نافذته وهي فوق الكل، وطلع الآتي:
  ///
  ///   • المحتوى أسود، **بس فقاعة الملاحظات ظاهرة فوقه**.
  ///     يعني الـ `Stack` بترسم أولادها الطايرين عادي — اللي مش بيترسم
  ///     هو `Positioned.fill(child: content)` لوحده.
  ///   • **صفر استثناءات وصفر overflow** في الكونسول. يبقى المشكلة في
  ///     الرسم مش في حساب المقاسات.
  ///   • `test/side_rail_layout_test.dart` بيثبت إن نفس التركيب
  ///     (Row → Expanded → Stack → IndexedStack → Navigator) بيتحسب
  ///     صح لوحده — فالسبب حاجة تانية في الشاشة الحقيقية.
  ///
  /// اللي اتجرّب وماحلّش: `Positioned.fill` على المحتوى · تحويل
  /// `Drawer` لـ `Material` عادي.
  ///
  /// ⚠️ الخيط اللي لسه ما اتتبعش: ليه أولاد الـ Stack الطايرين بيترسموا
  /// والمحتوى لأ؟ ده بيشاور على طبقة رسم (RepaintBoundary / Navigator
  /// جوّه IndexedStack) مش على تخطيط.
  ///
  /// البرنامج شغّال أهم من ميزة لسه مش مظبوطة — القايمة بتتفتح وتتقفل
  /// بزرار ☰ زي ما كانت.
  bool get hasSideRail => false;
