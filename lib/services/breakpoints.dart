...
extension BpContext on BuildContext {
  ...
  /// 🚫 مطفّية دلوقتي.
  ///
  /// لما كانت شغّالة، كان لازم نحط القايمة و المحتوى جنب بعض في `Row`.
  /// ده كان بيدّي **شاشة سودا** أول ما تفتح أي تاب على الشاشة العريضة
  /// (اتأكدت منها بالتصوير على تاب Samsung A9+ بالعرض = 1280 نقطة).
  ///
  /// جرّبت حاجتين ومنفعوش:
  ///   • `Positioned.fill` على المحتوى جوّه الـ Stack
  ///   • تغيير `Drawer` لـ `Material` عادي (الـ Drawer مصمّم للـ Scaffold بس)
  /// والعزل أثبت إن الـ `Row` نفسه هو السبب لما يكون في تاب مفتوح.
  ///
  /// البرنامج شغّال أهم من ميزة لسه مش مظبوطة — القايمة رجعت تتفتح وتتقفل
  /// بزرار ☰ زي ما كانت، لحد ما نلاقي السبب الجذري.
  /// جرّبتها تاني (٢٠٢٦-٠٨) بتشغيلها فعلاً على ويندوز بعرض ١٢٩٧ وتصويرها:
  /// الشاشة طلعت **سودا بالكامل** — حتى شريط القايمة نفسه ماظهرش. ومفيش
  /// ولا استثناء تخطيط في الكونسول، يعني المشكلة في الرسم مش في الحساب.
  /// واختبار `test/side_rail_layout_test.dart` بيثبت إن التركيب نفسه
  /// (Row → Expanded → Stack → IndexedStack → Navigator) سليم لوحده،
  /// فالسبب في حاجة تانية في الشاشة الحقيقية لسه مش متحدّدة.
